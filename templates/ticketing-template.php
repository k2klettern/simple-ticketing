                        ...
                        <form id="new-st" method="POST" action="">
							...
							if ( ! $action ) { ?>
								<?php if ( have_posts() ) : ?>
                                    ...
                                    <div id="tickets">
										<?php
										_e( 'A&uacute;n no existen Mensajes en tu Perfil', 'st_plugin' );
                                        echo StHelpers::getInstance()->build_footer_buttons(); ?>
                                    </div>
								<?php
								endif;
                                ...
                                <div class="form-group your-request">
									...
									wp_editor( $content, $editor_id, array(
										'textarea_name' => "input[0][message]",
										'teeny'         => true,
										'media_buttons' => false
									) ); ?>
                                </div>
                                <input type="hidden" name="input[0][userid]"
                                       value="<?php echo get_current_user_id(); ?>">
                                <input type="hidden" name="input[0][type]" value="mainmessage">
                                <button class="btn btn-md btn-submit" id="volver" onclick="history.back()"
                                        type="button"><i class="fa fa-chevron-left"></i> Volver
                                </button>
                                <input type="submit" class="btn btn-success btn-send-msg"
                                       value"<?php _e( 'Enviar', 'st_plugin' ); ?>">
								<?php
							} ?>
                        </form>
						<?php else : ?>
